...
async def call_llm(prompt: str, temperature: float = 0.3, max_tokens: int = 1024) -> str:
    """调用 LLM — 仅 WSL Qwen3.5-4B (GPU, :11435)。WSL离线则失败，等待重连。"""
    import httpx
    primary = llm_url or "http://127.0.0.1:11435/v1/chat/completions"
    for attempt, url in enumerate([primary], 1):
        try:
            ...
            import logging
            logging.getLogger("tmt").warning(f"LLM ({url}) 不可用: {e}")
            raise HTTPException(status_code=502, detail=f"LLM不可用 (WSL离线?): {e}")
    return ""
