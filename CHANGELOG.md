...
## 2026-06-07 — 发布 v3.5 🏷️

### 📋 版本管理修复
- 首次创建版本标签 `v3.5`
- 补充 `.gitignore` 排除 `__pycache__/` 和 `*.pyc`
- 更新 `日常运维/版本管理规范.md` — 提交格式+标签规则+工作流
- 新增 `架构设计/v3.5_优化需求与方案.md` — 基于使用健康审计的下版计划

### 🎯 下版计划 (v4.0)

| 优先级 | 方向 | 说明 |
|:------|:-----|:------|
| **P0** | 自动记忆召回 (Proactive Recall) | 对话中自动搜索相关记忆，注入 system prompt |
| P1 | 记忆足迹补全 | memory_traces 记录 store/recall |
| P1 | 会话上下文关联 | session_id 传递链路验证 |
| P2 | 实体解析 Pipeline | spaCy→GLiNER→LLM |
| P2 | 可视化 MVP | React + vis-network 图谱 |
