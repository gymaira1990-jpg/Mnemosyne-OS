...
推荐组合:
  Tier 2 → 豆包 Seed-2.0 Mini / gpt-4o-mini (快速/便宜)
  Tier 3 → 豆包 Seed-2.0 Lite (JSON mode, 蒸馏主力)
  Tier 4 → DeepSeek V4 Pro (深度推理/异构审计)
...
def _call_api(messages: list, model: str, max_tokens: int = 500,
    ...
    with urllib.request.urlopen(req, timeout=60) as resp:
        ...
        return {"content": data['choices'][0]['message']['content'], "tokens": data['usage']['total_tokens'], "model": model}
...
def call_llm(prompt: str, tier: int = 3, json_mode: bool = False,
    ...
    for _ in range(TMT_MAX_RETRIES):
        ...
        try:
            t0 = time.time()
            result = _call_api(messages, tinfo["model"], tinfo.get("max_tokens", 800),
            ...
            if json_mode:
                try:
                    ...
                    result["content"] = m.group() if m and json.loads(m.group()) or True else content
                    if not m: json.loads(content)
                except json.JSONDecodeError:
                    if current_tier < 4: current_tier += 1; continue
                    result["content"] = "{}"
            ...
            if len(_cache) >= MAX_CACHE: _cache.pop(next(iter(_cache)))
            _cache[cache_key] = final
            return final
        except Exception as e:
            last_error = e
            if current_tier < 4: current_tier += 1
            else: break
    return {"content": "", "tokens": 0, "model": "", "tier": 0, "cache_hit": False, "latency_ms": 0, "upgraded": False, "error": str(last_error)}
...
def get_cost_stats() -> dict:
    return {"by_tier": _cost_stats, "total_cost": 0}
...
def set_embed_cached(text: str, embedding: List[float]):
    if len(_embed_cache) >= MAX_CACHE: _embed_cache.pop(next(iter(_embed_cache)))
