...
class MnemosyneMemoryProvider(MemoryProvider):
    ...
    def system_prompt_block(self) -> str:
        ...
        try:
            ...
            parts = [
                ...
                f"用户: {self._user_id} | TMT: L1={l1}碎片 L2={l2}会话 L3={l3}日报 L4={l4}周报",
                f"总记忆: {total_mem} | 分类: {cat_str}",
            ]
            if hot_block:
                parts.append(
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        def _run():
            try:
                ...
                if len(query) > 15:
                    try:
                        dialectic = client.dialectic_search(query, limit=2)
                                ...
                                for c in ctx[:2]:
                                    if isinstance(c, dict):
                                        dialectic_parts.append(
                                            f"[{c.get('tier','?')}] {c.get('summary', str(c))[:150]}"
                                        )
                                if dialectic_parts:
    ...
    def on_pre_compress(self, messages: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            threading.Thread(target=_store_insight, daemon=True,
                            name="mnemosyne-precompress").start()
